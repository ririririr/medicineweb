...
import { MenuItem } from '../components/ai/AIPhoneMenu/types';

export const useAIMenu = (onClose: () => void, onTabChange: (tab: string) => void): MenuItem[] => {
  ...
  return [
    ...
    { 
      icon: User, 
      label: 'My Account',
      action: () => handleAction(() => onTabChange('account'))
    }
  ];
};